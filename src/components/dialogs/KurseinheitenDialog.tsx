...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Kurseinheiten';
...
interface KurseinheitenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Kurseinheiten['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Kurseinheiten['fields'];
...
export function KurseinheitenDialog({ open, onClose, onSubmit, defaultValues, recordId, trainerList, enablePhotoScan = true, enablePhotoLocation = true }: KurseinheitenDialogProps) {
  const [fields, setFields] = useState<Partial<Kurseinheiten['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['kursdatum', 'kursbezeichnung', 'trainer', 'dauer_stunden', 'stundensatz'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Kurseinheiten['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'kurseinheiten');
      await onSubmit(clean as Kurseinheiten['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Kurseinheiten['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Kurseinheiten bearbeiten' : 'Kurseinheiten hinzufügen';

  const fieldBlocks: Record<string, React.ReactNode> = {
    'kursdatum': (
      <div key="kursdatum" className="space-y-1.5">
        <Label htmlFor="kursdatum">Datum und Uhrzeit <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="kursdatum"
          placeholder="Wann findet der Kurs statt?"
          mode="datetime"
          value={fields.kursdatum ?? null}
          onChange={v => setFields(f => ({ ...f, kursdatum: v ?? undefined }))}
          required
        />
        {showErrors && !fields.kursdatum && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'kursbezeichnung': (
      <div key="kursbezeichnung" className="space-y-1.5">
        <Label htmlFor="kursbezeichnung">Kursbezeichnung <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="kursbezeichnung"
          placeholder="z. B. Yoga Anfänger"
          value={fields.kursbezeichnung ?? ''}
          onChange={e => setFields(f => ({ ...f, kursbezeichnung: e.target.value }))}
          required
        />
        {showErrors && !fields.kursbezeichnung && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'trainer': (
      <div key="trainer" className="space-y-1.5">
        <Label htmlFor="trainer">Trainer <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="trainer"
          placeholder="Welcher Trainer?"
    ...
    'dauer_stunden': (
      <div key="dauer_stunden" className="space-y-1.5">
        <Label htmlFor="dauer_stunden">Dauer (Stunden) <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="dauer_stunden"
          type="number"
          step="any"
        ...
        />
        {showErrors && !fields.dauer_stunden && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'stundensatz': (
      <div key="stundensatz" className="space-y-1.5">
        <Label htmlFor="stundensatz">Stundensatz (€) <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="stundensatz"
          type="number"
          step="any"
        ...
        />
        {showErrors && !fields.stundensatz && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'honorar': (
      <div key="honorar" className="space-y-1.5">
        <Label htmlFor="honorar">Honorar gesamt (€)</Label>
        <Input
          id="honorar"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'honorar')}
          placeholder="Wird automatisch berechnet"
          value={fields.honorar !== undefined ? fields.honorar : (computedValues['honorar'] ?? '')}
          onChange={e => setFields(f => ({ ...f, honorar: clampNumberValue(formEnhancements, 'honorar', e.target.value) }))}
        />
      </div>
    ),
    'abgerechnet': (
      <div key="abgerechnet" className="space-y-1.5">
        <Label htmlFor="abgerechnet">Bereits abgerechnet</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="abgerechnet"
            checked={!!fields.abgerechnet}
            onCheckedChange={(v) => setFields(f => ({ ...f, abgerechnet: !!v }))}
          />
          <Label htmlFor="abgerechnet" className="font-normal">Bereits abgerechnet</Label>
        </div>
      </div>
    ),
    'bemerkung': (
  ...
  const FIELD_LABELS: Record<string, string> = {"kursdatum": "Datum und Uhrzeit", "kursbezeichnung": "Kursbezeichnung", "trainer": "Trainer", "dauer_stunden": "Dauer (Stunden)", "stundensatz": "Stundensatz (€)", "honorar": "Honorar gesamt (€)", "abgerechnet": "Bereits abgerechnet", "bemerkung": "Bemerkung"};
  const CURRENCY_KEYS = new Set<string>(["stundensatz", "honorar"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"trainer": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "standard_stundensatz": "Standard-Stundensatz (€)"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.KURSEINHEITEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createTrainerOpen && (
      <TrainerDialog
        ...
        defaultValues={createTrainerInitial
          ? ({ vorname: createTrainerInitial } as any)
          : undefined}
      />
    )}
    </>
