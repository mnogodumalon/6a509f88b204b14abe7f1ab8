...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Trainer';
...
export function TrainerDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: TrainerDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Trainer['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'trainer');
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'standard_stundensatz': (
      <div key="standard_stundensatz" className="space-y-1.5">
        <Label htmlFor="standard_stundensatz">Standard-Stundensatz (€)</Label>
        <Input
          id="standard_stundensatz"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'standard_stundensatz')}
          placeholder="z. B. 45"
          value={fields.standard_stundensatz !== undefined ? fields.standard_stundensatz : (computedValues['standard_stundensatz'] ?? '')}
          onChange={e => setFields(f => ({ ...f, standard_stundensatz: clampNumberValue(formEnhancements, 'standard_stundensatz', e.target.value) }))}
        />
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "standard_stundensatz": "Standard-Stundensatz (€)"};
  const CURRENCY_KEYS = new Set<string>(["standard_stundensatz"]);
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.TRAINER} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
