...
import { KurseinheitenViewDialog } from '@/components/dialogs/KurseinheitenViewDialog';
...
const TRAINER_FIELDS = [
  ...
  { key: 'standard_stundensatz', label: 'Standard-Stundensatz (€)', type: 'number' },
];
const KURSEINHEITEN_FIELDS = [
  { key: 'kursdatum', label: 'Datum und Uhrzeit', type: 'date/datetimeminute' },
  { key: 'kursbezeichnung', label: 'Kursbezeichnung', type: 'string/text' },
  ...
  { key: 'stundensatz', label: 'Stundensatz (€)', type: 'number' },
  { key: 'honorar', label: 'Honorar gesamt (€)', type: 'number' },
  { key: 'abgerechnet', label: 'Bereits abgerechnet', type: 'bool' },
  { key: 'bemerkung', label: 'Bemerkung', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'trainer', label: 'Trainer', pascal: 'Trainer' },
  { key: 'kurseinheiten', label: 'Kurseinheiten', pascal: 'Kurseinheiten' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('trainer');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'trainer': new Set(),
    'kurseinheiten': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'kurseinheiten': return (data as any).kurseinheiten as Kurseinheiten[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'kurseinheiten' && fieldKey === 'trainer') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'kurseinheiten': return KURSEINHEITEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'kurseinheiten': return {
        create: (fields: any) => LivingAppsService.createKurseinheitenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateKurseinheitenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteKurseinheitenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'kurseinheiten' || dialogState?.entity === 'kurseinheiten') && (
        <KurseinheitenDialog
          open={createEntity === 'kurseinheiten' || dialogState?.entity === 'kurseinheiten'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'kurseinheiten' ? handleUpdate : (fields: any) => handleCreate('kurseinheiten', fields)}
          defaultValues={dialogState?.entity === 'kurseinheiten' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'kurseinheiten' && (
        <KurseinheitenViewDialog
          open={viewState?.entity === 'kurseinheiten'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'kurseinheiten', record: r }); }}
          trainerList={(data as any).trainer ?? []}
        />
      )}
