...
import { formEnhancements } from '@/config/form-enhancements/Trainer';
...
export default function TrainerDetailPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getTrainer();
  ...
  async function handleUpdate(fields: Trainer['fields']) {
    if (!record) return;
    await LivingAppsService.updateTrainerEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteTrainerEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/trainer');
  }
  ...
  return (
    <RecordView
      onBack={() => navigate('/trainer')}
      onEdit={() => setEditing(true)}
      backLabel="Zurück"
      editLabel="Bearbeiten"
    >
      <RecordHeader title={record.fields.vorname ?? 'Trainer'} />
      ...
      <RecordSection title="Details" cols={2}>
        ...
        <RecordField label="Standard-Stundensatz (€)" value={record.fields.standard_stundensatz} format="text" />
      </RecordSection>

      <RecordAttachments appId={APP_IDS.TRAINER} recordId={record.record_id} />
