...
export default function TrainerPage() {
  ...
  const [editingRecord, setEditingRecord] = useState<Trainer | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getTrainer());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Trainer['fields']) {
    await LivingAppsService.createTrainerEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Trainer['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateTrainerEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteTrainerEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Trainer"
      subtitle={`${records.length} Trainer im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('standard_stundensatz')}>
                <span className="inline-flex items-center gap-1">
                  Standard-Stundensatz (€)
                  {sortKey === 'standard_stundensatz' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/trainer/${record.record_id}`); }}>
                ...
                <TableCell>{record.fields.standard_stundensatz ?? '—'}</TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Trainer. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
