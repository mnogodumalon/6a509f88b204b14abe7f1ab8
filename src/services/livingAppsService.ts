...
import type { Trainer, Kurseinheiten, CreateTrainer, CreateKurseinheiten } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- KURSEINHEITEN ---
  static async getKurseinheiten(): Promise<Kurseinheiten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSEINHEITEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Kurseinheiten[];
    return enrichLookupFields(records, 'kurseinheiten');
  }
  static async getKurseinheitenEntry(id: string): Promise<Kurseinheiten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSEINHEITEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Kurseinheiten;
    return enrichLookupFields([record], 'kurseinheiten')[0];
  }
  static async createKurseinheitenEntry(fields: CreateKurseinheiten) {
    return callApi('POST', `/apps/${APP_IDS.KURSEINHEITEN}/records`, { fields: cleanFieldsForApi(fields as any, 'kurseinheiten') });
  }
  static async updateKurseinheitenEntry(id: string, fields: Partial<CreateKurseinheiten>) {
    return callApi('PATCH', `/apps/${APP_IDS.KURSEINHEITEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kurseinheiten') });
  }
  static async deleteKurseinheitenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.KURSEINHEITEN}/records/${id}`);
  }
